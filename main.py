...
def detector():
    ...
    # minimum duration of the cursor movement
    pydirectinput.MINIMUM_DURATION = 0.000001
    ...
    while True:
        ...
        if getForegroundWindow() == "Motion Tracking Game" or getForegroundWindow() == "Motion Tracker":
            ...
            cv2.putText(img, str(int(fps)), (10, 70), cv2.FONT_HERSHEY_COMPLEX,
                        3, (0, 255, 0), 3)  # put the fps on the image

            if len(lm) > 0:  # if a hand is detected
                MiddleFinger = Vector.Vector3(
                    lm[12][1], lm[12][2], 0)  # middle finger
                FirstFinger = Vector.Vector3(
                    lm[8][1], lm[8][2], 0)  # index finger
                ...
                Cx = Vector.clamp(0.75*lm[9][1]/MouseSpeedX, 0, 1)
                # clamp the y position
                Cy = Vector.clamp(0.75*lm[9][2]/MouseSpeedY, 0, 1)
                pdistance = Distance  # set the previous distance
                # set the mouse destination
                MouseDestinationx = pyautogui.size(
                )[0]-(Cx * pyautogui.size()[0])
                MouseDestinationy = Cy * pyautogui.size()[1]
                # move the cursor towards destination
                pydirectinput.moveTo(int(MouseDestinationx), int(
                    MouseDestinationy), CursorSpeed, _pause=False)
            ...
            MouseSpeedX = cv2.getWindowImageRect('Motion Tracker')[
            ...
            try:
                # if the current foreground window is not the one we want
                cv2.destroyWindow("Motion Tracker")
            except:
                pass
            _, img = cam.read()
